...
class AsianCallFun(fun):
    ...
    def __init__(self, BMmeasure=None):
        ...
        if self.BMmeasure:
            ...
            for i in range(1,len(nBM)):
                self[i].BMmeasure = self.BMmeasure[i]
                self[i].dimFac = self.BMmeasure[i].dimension/self.BMmeasure[i-1].dimension
                self[i].dimension = self.BMmeasure[i].dimension  
