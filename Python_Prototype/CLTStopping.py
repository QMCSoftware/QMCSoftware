...
from numpy import zeros, full, inf, sqrt, ones, kron
...
class CLTStopping(stoppingCriterion):
    ...
    def  stopYet(self, funObj, distribObj: IIDDistribution, dataObj = meanVarData()):  # defaults dataObj to meanVarData if not supplied by user
        if dataObj.stage == 'begin': # initialize
            ...
            nM = ceil((tempB*(self.quantile*self.inflate/max(self.absTol,dataObj.solution*self.relTol))^2)* (dataObj.sighat./sqrt(dataObj.costF)))
            ...
            errBar = (self.quantile * self.inflate) * sqrt(sum(dataObj.sighat.^2/dataObj.nMu))
