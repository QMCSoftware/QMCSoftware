...
class Test_Measure(unittest.TestCase):
    def test_measure_construction(self):
        ...
        with self.subTest():
            self.assertEqual(measure[1].dimension, 16)
        with self.subTest():
            self.assertEqual(type(measure[1]).__name__, "BrownianMotion")
        with self.subTest():
            self.assertTrue(hasattr(measure[1], "time_vector"))
        with self.subTest():
            self.assertEqual(measure[1].time_vector[0], 1 / 16)
