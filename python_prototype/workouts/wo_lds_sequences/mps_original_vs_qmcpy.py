...
def mps_gentimes(n_2powers=arange(1,11)):
    """
    Record wall-clock time for generating samples from
    ...
    for i, n_2 in enumerate(n_2powers):
        ...
        t0 = time()
        lattice_rng = latticeseq_b2(m=30, s=dim)
        mps_lattice_samples = array([next(lattice_rng) for i in range(n_samples)])
        row_i['mps_lattice_time'] = time() - t0
        # Refactored MPS Lattice
        t0 = time()
        lattice_rng = LatticeSeq(m=30, s=dim)
        qmcpy_lattice_samples = array([next(lattice_rng) for i in range(n_samples)])
        row_i['qmcpy_lattice_time'] = time() - t0
        # Original MPS Sobol
        t0 = time()
        sobol_rng = digitalseq_b2g(Cs="./third_party/magic_point_shop/sobol_Cs.col", m=30, s=dim)
        mps_Sobol_samples = zeros((n_samples, dim), dtype=int64)
        for i in range(n_samples):
            next(sobol_rng)
            mps_Sobol_samples[i, :] = sobol_rng.cur
        row_i['mps_Sobol_time'] = time() - t0
        ...
        row_i['qmcpy_Sobol_time'] = time() - t0
...
if __name__ == '__main__': 
    df_times = mps_gentimes(n_2powers=arange(1,21))
