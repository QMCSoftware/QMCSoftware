...
def clt_iidstduniform(dimension, abs_tol, rel_tol):
    ...
    algorithm = CLT(distribution, abs_tol=abs_tol, rel_tol=rel_tol)
    return integrate(algorithm,integrand,measure,distribution)[1]
...
def cltrep_lattice(dimension, abs_tol, rel_tol):
    ...
    measure = Gaussian(distribution, variance=1/2)
    integrand = Keister(measure)
    algorithm = CLTRep(distribution,abs_tol=abs_tol, rel_tol=rel_tol)
    return integrate(algorithm,integrand,measure,distribution)[1]
...
def meanmc_g_iidstduniform(dimension, abs_tol, rel_tol):
    ...
    measure = Gaussian(distribution, variance=1/2)
    integrand = Keister(measure)
    algorithm = MeanMC_g(distribution,abs_tol=abs_tol, rel_tol=rel_tol)
    return integrate(algorithm,integrand,measure,distribution)[1]
...
def cublattice_g(dimension, abs_tol, rel_tol):
    ...
    measure = Gaussian(distribution, variance=1/2)
    integrand = Keister(measure)
    algorithm = CubLattice_g(distribution, abs_tol=abs_tol, rel_tol=rel_tol)
    return integrate(algorithm,integrand,measure,distribution)[1]
