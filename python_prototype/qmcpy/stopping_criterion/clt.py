...
class CLT(StoppingCriterion):
    ...
    def __init__(self, discrete_distrib, true_measure,
        ...
        self.data._n_prev = zeros(n_integrands)  # initialize data object
        self.data._n_next = tile(self.n_init, n_integrands)
        # initialize as n_init

    def stop_yet(self):
        """ Determine when to stopping_criterion """
        if self.stage == "sigma":
            self.data._n_prev = self.data._n_next  # update place in the sequence
            ...
            # n_mu := n_mu_temp adjusted for _n_prev
            self.data.n_mu = minimum(maximum(self.data._n_next, n_mu_temp),
                                     self.n_max - self.data._n_prev)
            self.data._n_next = (self.data.n_mu + self.data._n_prev)
