...
def MyHOSobol(m, s, d=1, dat_file='sobol.dat'):
    """
    ...
    Args:
        ...
        dat_file: first-order Sobol points
   ...
   array([[0.]])
    ...
    """
    import os
    filepath = os.getcwd()+os.sep+"qmcpy/machine_learning/compression"
    #print(f"filepath = {filepath}")
    if os.sep in dat_file:
        z = np.loadtxt(dat_file)
    else:
        z = np.loadtxt(filepath + os.sep + dat_file)
    z = z[:2 ** m, :s * d]
    if d > 1:
