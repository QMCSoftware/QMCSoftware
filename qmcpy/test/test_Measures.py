...
class Test_Measure(unittest.TestCase):
    ...
    def test_measure_construction(self):
        measure = BrownianMotion(time_vector=[arange(1/4,5/4,1/4),arange(1/16,17/16,1/16)])
        ...
        with self.subTest(): self.assertTrue(hasattr(measure[1],'time_vector'))
        with self.subTest(): self.assertEqual(measure[1].time_vector[0],1/16)
