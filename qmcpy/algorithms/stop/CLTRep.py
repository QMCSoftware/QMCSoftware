...
class CLTRep(StoppingCriterion):
    ...
    def __init__(self, distribution, n_streams=16, inflate=1.2, alpha=0.01, abs_tol=1e-2, rel_tol=0, n_init=1024, n_max=1e8):
        """
        Args:
    ...
    def stopYet(self, integrand):
        ...
        if self.data_obj.flag.sum()==0 or self.data_obj.n_next.max() > self.n_max:
            ...
            self.data_obj.confid_int = self.data_obj.solution + errBar * array([-1, 1]) # CLT confidence interval
