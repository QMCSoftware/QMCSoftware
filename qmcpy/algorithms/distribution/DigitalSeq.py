...
class DigitalSeq(digitalseq_b2g):
    ...
    def __init__(self, Cs, kstart=0, m=None, s=None, returnDeepCopy=True):
        """
          ...
          Inputs:
            Cs      generating matrices as a list of lists, see description
            above;
          ...
          construction scripts by using:
              numpy.loadtxt('Bs64.col', int) # mind the int: read as
              integers! but: machine integers!
        ...
        if isinstance(Cs, basestr):
            # filename passed in
            Cs = pd.read_csv(Cs, header=None, delimiter=" ",
