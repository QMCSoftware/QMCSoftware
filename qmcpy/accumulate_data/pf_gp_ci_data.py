...
class PFGPCIData(AccumulateData):
    ...
    def __init__(self, stopping_crit, integrand, true_measure, discrete_distrib, dnb2, n_approx, alpha, refit, 
        ...
        torch_optimizer_func,gpytorch_train_iter,gpytorch_use_gpu,
        verbose,approx_true_solution,ref_approx):
        self.parameters = ['solution','error_bound','n_total','time_integrate']
    ...
    def update_data(self, batch_count, xdraw, ydrawtf):
        ...
        self.n_batch.append(len(ydrawtf))
        ...
        if batch_count==0 or self.refit:
            ...
            gpyt_model = self.gpyt_model.add_data(xdraw,ydrawtf)
    ...
    def get_results_dict(self):
        df = {
            'n_sum': cumsum(self.n_batch),
            'n_batch': array(self.n_batch),
            'error_bounds': array(self.error_bounds),
            'ci_low': array(self.ci_low),
            'ci_high': array(self.ci_high),
            'solutions': array(self.solutions)}
        if self.approx_true_solution:
            df['ref_approx'] = tile(self.ref_approx,len(self.n_batch))
            df['error_abs'] = abs(df['solutions']-df['ref_approx'])
            df['in_ci'] = (df['ci_low']<=df['solutions'])*(df['solutions']<=df['ci_high'])
        return df 
    
    def plot(self, df, x, y, figroot='mcispfgp.png', trace_only=False, **kwargs):
        from matplotlib import pyplot
        import os
        pyplot.style.use(os.path.dirname(os.path.abspath(__file__))+'/mcispfgp.mplstyle')
        if self.d==1 and not trace_only and self.saved_gps!=[]: fig,gs = self.plot_1d(df,x,y,**kwargs)
        elif self.d==2 and not trace_only and self.saved_gps!=[]: fig,gs = self.plot_2d(df,x,y,**kwargs)
        else:
            from matplotlib import pyplot,gridspec
            fig = pyplot.figure(constrained_layout=False,figsize=(8,4))
            gs = gridspec.GridSpec(1,2,figure=fig)
        axtrace = fig.add_subplot(gs[-1,:])
        if self.approx_true_solution: axtrace.plot(.5+arange(len(df)),tile(self.ref_approx,len(df)),color='c',label=r'$P(g)$')
        axtrace.plot(.5+arange(len(df)),df['approx'],'-o',color='k',label=r'$\hat{P}_n^\mathrm{QMC}$')
        axtrace.fill_between(.5+arange(len(df)),df['ci_low'],df['ci_high'],color='k',alpha=.15)
        axtrace.plot(.5+arange(len(df)),df['error_ci'],'-o',color='m',label=r'$\hat{\gamma}_n^\text{QMC}$')
        if self.approx_true_solution: axtrace.plot(.5+arange(len(df)),df['error_abs'],'-o',color='b',label=r'$\vert \hat{P}_n^\text{QMC} - P(g) \vert$')
        axtrace.set_xlim([0,len(df)])
        axtrace.set_xticks([])
        ...
        fig.savefig(figroot,dpi=256,transparent=False)
        return fig
    def plot_1d(self, df, x, y, meshticks=1025, ci_percentage=.95, **kwargs):
        from matplotlib import pyplot,gridspec
        ...
        if self.approx_true_solution: yticks = self.integrand(xticks[:,None])
        for j in range(n_batches):
            i0,i1 = 0 if j==0 else df['n_sum'][j-1],df['n_sum'][j]
            ...
            pfx_udens = self._pfx_udens(gpyt_model,xticks[:,None])
            mr_udens = self._ewmr_udens_from_pfx(pfx_udens)
            ax0j.fill_between(xticks,mr_udens,color='k',alpha=.5)
            ax0j.scatter(x[i0:i1].squeeze(),zeros(i1-i0)+.015,color='r',s=25)
            ax0j.set_ylim([0,1])
            ax0j.set_yticks([0,1])
            gpyt_model = ExactGPyTorchRegressionModel(
                x_t = x[:i1], y_t = y[:i1],
            ...
            if self.approx_true_solution: ax1j.plot(xticks,yticks,color='c',label='f(x)',linewidth=2)
            ...
            ax1j.scatter(x[:i0].squeeze(),y[:i0],color='b',label='old pts')
            ax1j.scatter(x[i0:i1].squeeze(),y[i0:i1],color='r',label='new pts')
            ax1j.set_xlabel(r'$u$')
            if j==0:
                #ax1j.set_ylabel(r'$y$')
        ...
        for ax in fig.axes:
            ...
            ax.set_xticks([0,1])
        return fig,gs
    def plot_2d(self, df, x, y, meshticks=257, clevels=32, **kwargs):
        ...
        if self.approx_true_solution:
            yquery = self.integrand(xquery)
            ymesh = yquery.reshape(x0mesh.shape)
        for j in range(n_batches):
            row_idx = 0
            i0,i1 = 0 if j==0 else df['n_sum'][j-1],df['n_sum'][j]
            if self.approx_true_solution:
                ax0j = fig.add_subplot(gs[row_idx,j])
                ax0j.contourf(x0mesh,x1mesh,ymesh,cmap=cm.Greys,vmin=ymesh.min(),vmax=ymesh.max(),levels=clevels)
                ax0j.contour(x0mesh,x1mesh,ymesh,colors='lightgreen',levels=[0],linewidths=5)
            ...
            udens_mr = self._ewmr_udens(gpyt_model,xquery).reshape(x0mesh.shape)
            ax1j.contourf(x0mesh,x1mesh,udens_mr,cmap=cm.Greys,levels=clevels,vmin=0,vmax=1)
            ax1j.scatter(x[i0:i1,0],x[i0:i1,1],color='r')
            ...
            for ax in [ax2j,ax3j]:
                ax.scatter(x[:i0,0],x[:i0,1],color='b')
                ax.scatter(x[i0:i1,0],x[i0:i1,1],color='r')
        for ax in fig.axes:
            ax.set_xlim([0,1])
            ax.set_ylim([0,1])
            ax.set_xlabel(r'$u_1$')
