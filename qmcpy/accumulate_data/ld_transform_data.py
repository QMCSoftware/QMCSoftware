...
class OmgHat:
    def __init__(self, fudge, r_lag, omg_circ):
        self.fudge = fudge
        self.r_lag = r_lag
        self.omg_circ = omg_circ

    def __call__(self, m):
        return self.fudge(m) / ((1 + self.fudge(self.r_lag)) * self.omg_circ(self.r_lag))
...
class LDTransformData(AccumulateData):
    ...
    def __init__(self, m_min, m_max, coefv, fudge, check_cone, ncv, cv_mu, update_beta):
        ...
        self.omg_circ = omg_circ_func
        self.r_lag = 4
        self.l_star = int(self.m_min-self.r_lag)
        self.omg_hat = OmgHat(self.fudge, self.r_lag, self.omg_circ)
