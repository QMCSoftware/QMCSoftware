...
class AbstractStoppingCriterion(object):
    ...
    def _validate_resume_data(self, data, required_fields=()):
        self._require_resume_attrs(
            data,
            ("stopping_crit", "integrand", "true_measure", "discrete_distrib", "n_total")
            + tuple(required_fields),
        )
        if type(data.stopping_crit) is not type(self):
            raise ParameterError(
                "resume data was generated by %s, not %s."
                % (type(data.stopping_crit).__name__, type(self).__name__)
            )
        self._validate_resume_object(
            "integrand",
            self.integrand,
            data.integrand,
            ("d", "d_indv", "d_comb") + tuple(getattr(self.integrand, "parameters", [])),
        )
        self._validate_resume_object(
            "true_measure",
            self.true_measure,
            data.true_measure,
            ("d", "domain", "range")
            + tuple(getattr(self.true_measure, "parameters", [])),
        )
        self._validate_resume_object(
            "discrete_distrib",
        ...
        if int(data.n_total) > int(self.n_limit):
            raise ParameterError(
                "resume data n_total=%d exceeds current n_limit=%d."
                % (int(data.n_total), int(self.n_limit))
            )
