...
class IndepVecVGPLatents(gpytorch.models.ApproximateGP):
    """
    https://docs.gpytorch.ai/en/latest/examples/04_Variational_and_Approximate_GPs/SVGP_Multitask_GP_Regression.html
    """
    def __init__(self, num_tasks, num_inducing_pts, dimension, num_latents, mean_module=None, covar_module=None):
        inducing_points = torch.rand(num_latents, num_inducing_pts, dimension)
        ...
        if covar_module is None:
            self.covar_module = gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.RQKernel(batch_shape=torch.Size([num_latents]),ard_num_dims=dimension),
