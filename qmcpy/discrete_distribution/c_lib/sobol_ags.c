...
void sobol_ags(unsigned long n, unsigned int d, unsigned long n0, unsigned int d0, unsigned int randomize, unsigned int graycode, unsigned long seed, double *x, unsigned int d_max, unsigned int m_max, unsigned long *z, unsigned int msb){
    /*
    Custom Sobol' Generator by alegresor

    n: sample indicies include n0:(n0+n)
    d: dimension includes d0:(d0+d)
    n0: starting index in sequence
    ...
    for(i=n0;i<(n0+n);i++){
        ...
        if((randomize==1) || (randomize==2)){
            ...
            for(j=0;j<d;j++){
                xr[j] = xr[j] ^ rshift[j];}}
        for(j=0;j<d;j++){
            x[(i-n0)*d+j] = ((double) xr[j])*scale;}}}

int main(){
    unsigned long n = 1<<25;
    unsigned int d = 2;
    unsigned long n0 = 0; 
    unsigned int d0 = 0;
    unsigned int randomize = 0;
    unsigned int graycode = 1;
    ...
    unsigned int msb = 0;
    */
    sobol_ags(n, d, n0, d0, randomize, graycode, seed, x, d_max, m_max, *z, msb);
    /* print result 
