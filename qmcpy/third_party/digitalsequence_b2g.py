...
class digitalseq_b2g:
    ...
    def __init__(self, Cs, kstart=0, m=None, s=None, returnDeepCopy=True):
        ...
        # Extend precision if necessary
        if self.t > 53:
            mpmath.mp.prec = self.t
            self.recipd = mpmath.mpf(1) / 2**self.t
...
from numpy import array,log,random,int64,zeros
def get_RS_sobol_b2g(n,m,j):
    # generates j shifted nxm sobol digital sequences
    gen = digitalseq_b2g(Cs='./third_party/sobol_Cs.col',m=int(log(n)/log(2)),s=m)
