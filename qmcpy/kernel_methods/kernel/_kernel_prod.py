...
class _KernelProd(object):
    def __init__(self, dimension, lengthscales=1., scale=1., torchify=False, alpha=None, **kwargs):
        """
        Args:
        ...
        assert alpha is None or isinstance(alpha,int) or alpha.shape==(self.d,)
        if isinstance(alpha,int):
            alpha = alpha*self.npt.ones(self.d,dtype=int)
        self.alpha = alpha
