    ...
    \begin{column}{\twocolwid}
     % \vspace{-2ex}
        \begin{figure} 
            ...
            \caption{\ Reducing tolerance resulting in more samples and better approximations of the Keister function~\cite{keister1996multidimensional}, $f(\vx) = \pi^{d/2} \cos(\lVert \vx \rVert)$ for $\vx \in  \mathbb{R}^d$ with $d=2$ integrated over  the unit cube. This figure is reproducible by \texttt{Test\_3D\_Point\_Distribution.py}~\cite{HicEtal19}.}  \label{fig3d}
        \end{figure} 
    \end{column}
    \begin{column}{\onecolwid}
        %\inputminted{python}{Keister.snip.py}
        \lstinputlisting[style=Python,caption=Code for generating the first subplot in Figure~1.]{Keister.snip.py}
    \end{column}
