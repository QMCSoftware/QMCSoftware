"""
pytest --doctest-modules qmcpy/discrete_distribution/digital_net_b2/digital_net_b2.py
python test/test_discrete_distribs.py
"""
...
PATHTOGMS = "./qmcpy/discrete_distribution/digital_net_b2"

class DigitalNetB2Original(qp.discrete_distribution.abstract_discrete_distribution.AbstractLDDiscreteDistribution):
    r"""
    ...
    **References:**
    ...
    def __init__(
        ...
        if (
            ...
            if np.isscalar(dimension) and dimension <= 1024 and alpha == 1:
                gen_mats = np.load(
                    PATHTOGMS#dirname(abspath(__file__))
        ...
        super(DigitalNetB2Original, self).__init__(
    ...
    def _gen_samples(self, n_min, n_max, return_binary, warn):
        ...
        d = (
            np.uint64(self.dtalpha)
            if self.randomize == "NUS" and self.alpha > 1
            else np.uint64(self.d)
        )
        ...
        if "NUS" in self.randomize:
            if self.alpha == 1:
                ...
                qmctoolscl.dnb2_nested_uniform_scramble(
                    r,
                    n,
                    d,
                    r_x,
                ...
                _ = qmctoolscl.dnb2_nested_uniform_scramble(
                    r,
                    n,
                    dtalpha,
                    r_x,
        ...
        if "DS" in self.randomize:
            ...
            qmctoolscl.dnb2_digital_shift(
                r, n, d, r_x, lshifts, xb, self.rshift, xrb, backend="c"
            )
            xb = xrb
        if return_binary:
            return xb
...
if __name__=="__main__":
    ...
    times_dnb2_original = np.empty(repeats)
    times_dnb2_new = np.empty(repeats)
    times_dnb2_scipy = np.empty(repeats)
    t0_overall = time.perf_counter()
    print("repeat of %d: "%repeats,end='',flush=True)
    for i in range(repeats):
        ...
        # scipy 
        t0 = time.perf_counter()
        _ = scipy.stats.qmc.Sobol(d,scramble=True).random_base2(m=m)
        times_dnb2_scipy[i] = time.perf_counter()-t0
    print("\n")

    df = pd.DataFrame({
        "original": times_dnb2_original,
        "new": times_dnb2_new,
        "scipy": times_dnb2_scipy,
        "speedup_original_new": times_dnb2_original/times_dnb2_new,
        "speedup_scipy_original": times_dnb2_scipy/times_dnb2_original,
        "speedup_scipy_new": times_dnb2_scipy/times_dnb2_new,
    })
    df_summary = df.describe()
    df.to_csv("compare_dnb2_times.df.csv")
    df_summary.to_csv("compare_dnb2_times.summary.csv")
    print(df_summary)
    print()
    print(df)
