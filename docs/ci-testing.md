...
| `unittests.yml` (`core-tests` job) | same as above | `ubuntu`; Python `3.9` | <ul><li>Build and install the no-extra user wheel, check dependencies, and import outside the source tree</li><li>Install `test_core`, then run `unittests_core` (no booktests)</li><li>Blocking test of the declared support-policy floor</li></ul> |
