...
def main():
    notebooks = glob.glob(os.path.join(DEMOS_PATH, '*.ipynb'), recursive=True)
    tasks = []
    ...
    for nb in notebooks:
        ...
        if name in exclude:
            print("Skipping", name)
            continue
        tasks.append((name, run_notebook(nb, project_root)))
    ...
    for name, t in tasks:
        try:
            ...
            print(f"Error in notebook {name}: {e}")
    ...
    for r in results:
        print(" -", r)
    if errors:
        print("Errors encountered in the following notebooks:")
        for name, err in errors:
            print(f" - {name}: {err}")

if __name__ == '__main__':
    main()
    # time python /Users/terrya/Documents/ProgramData/QMCSoftware/test/ntbktests/all_notebook_tests_subprocesses.py
    # 12 notebooks: python   0.47s user 0.43s system 0% cpu 2:15.51 total    
    # all notebooks: python   1.62s user 1.85s system 0% cpu 12:42.55 total
    # Multiproccesses on Mac
    # No testbook. Used nbconvert.