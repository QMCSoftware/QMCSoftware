...
# Local execution with HighThroughputExecutor for Mac
config = Config(
    executors=[
        HighThroughputExecutor(
            label="htex_local",
            cores_per_worker=1,   # Use 1 core per worker for notebook execution
            provider=LocalProvider(
                init_blocks=1,    # Start with 1 block
                max_blocks=3,     # Allow up to 2 parallel notebooks
            ),
        )
    ],
    strategy=None,
