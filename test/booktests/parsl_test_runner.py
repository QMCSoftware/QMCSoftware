...
def execute_parallel_tests():
    ...
    if len(unique_test_modules) < len(test_modules):
        print(
            f"Note: Removed {len(test_modules) - len(unique_test_modules)} duplicate test module(s)"
        )
    ...
    for module, future, index in futures:
        ...
        if mem_match:
            ...
            print(f"[{completed}/{len(futures)}] {module}: {status}")
            print(
                f"{test_name} ...     Memory used: {mem_used} GB.  Test time: {test_time} s\n{ok_status}"
            )
        else:
            print(f"[{completed}/{len(futures)}] {module}: {status}")
...
def generate_summary_report(results, execution_time=0.0, total_test_time=0.0):
    ...
    total_skipped = sum(
        skip_count for _, status, skip_count in results if status == "PASSED"
    )
        ...
        try:
            ...
            if platform.system() == "Darwin":
                ...
                config = Config(
                    executors=[
                        ThreadPoolExecutor(
                            max_threads=max_workers, label="local_threads"
                        )
                    ]
                )
            else:
