...
class NotebookTests(BaseNotebookTest):
    ...
    def setUp(self):
        ...
        demo_dir = os.path.join(
            os.path.dirname(__file__), "..", "..", "demos", "DAKOTA_Genz"
        )
        ...
        target_dakota_file = os.path.join(
            os.path.dirname(__file__), "x_full_dakota.txt"
        )

        if os.path.isfile(source_dakota_file) and not os.path.isfile(
            target_dakota_file
        ):
            ...
            dummy_data = np.random.rand(
                1024, 128
            )  # Much smaller than ns.max() = 262144
    ...
    @testbook(
        "../../demos/DAKOTA_Genz/dakota_genz.ipynb", execute=False, timeout=TB_TIMEOUT
    )
