...
for dirpath, dirnames, filenames in os.walk(root):
    ...
    if any(skip in dirpath for skip in ['.git', '__pycache__', 'build', 'lib.macosx', 'qmcpy.egg-info']):
        continue
    for fn in filenames:
        if fn.endswith('.md') or fn.endswith('.rst') or fn.endswith('.ipynb'):
            ...
            if single_dollars % 2 == 1:
                problem_files.append((path, total_dollars, double_dollars, single_dollars))
