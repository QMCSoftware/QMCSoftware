...
\title{QMCPy 2.0: An Open-Source Python Framework for (Quasi-)Monte
Carlo Algorithms}
...
enables researchers to easily implement novel (Q)MC algorithms. The
framework offers user-friendly APIs, diverse (Q)MC algorithms, adaptive
error estimation, and integration with scientific libraries following
reproducible research practices. Compared to previous versions,
\texttt{QMCPy} 2.0 includes:
...
identically distributed (IID) sequences and QMC methods which use LD
sequences that more evenly cover the unit cube, enabling faster
convergence \citep{Ric51}. \autoref{fig:points} visualizes IID and LD
sequences.

\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio,alt={An IID sequence with gaps and clusters alongside LD sequences which more evenly fill the space. Each of the three randomized LD sequences contains purple stars (initial 32 points), green triangles (next 32), and blue circles (subsequent 64). The lattice was randomly shifted; the digital sequence was randomized with nested uniform scrambling (Owen scrambling); the Halton sequence was randomized with linear matrix scrambling and permutation scrambling.}]{../demos/talk_paper_demos/JOSS2025/JOSS2025.outputs/points.png}
\caption{An IID sequence with gaps and clusters alongside LD sequences
which more evenly fill the space. Each of the three randomized LD
sequences contains purple stars (initial 32 points), green triangles
(next 32), and blue circles (subsequent 64). The lattice was randomly
shifted; the digital sequence was randomized with nested uniform
scrambling (Owen scrambling); the Halton sequence was randomized with
...
\citep{ChoEtal21a2, TonEtAl22a}, with both adhering to reproducible
research practices \citep{Cho14a2, ChoEtal22a}. However, \texttt{QMCPy}
consolidates a substantially broader range of cutting-edge (Q)MC
algorithms
...
\end{equation} where \(g\) is the \textbf{integrand} and \(\lambda\) is
the probability density of a random variable \(\mathbf{T}\) whose
distribution we call the \textbf{true measure}. Through an appropriate
transformation \(\boldsymbol{\psi}\), we rewrite \(\mu\) as the
expectation of a function of a standard uniform random variable
...
\end{equation} If \(\boldsymbol{\psi}\) satisfies \(\mathbf{T} \sim
...
\end{equation} MC methods choose IID sampling nodes
\(\mathbf{X}_1,\dots,\mathbf{X}_n\) and have error
\(|\widehat{\mu}-\mu|\) like \(\mathcal{O}(n^{-1/2})\) \citep{Nie78}.
QMC methods choose dependent LD nodes that fill \([0,1]^d\) more evenly,
achieving errors like \(\mathcal{O}(n^{-1+\delta})\) where \(\delta>0\)
is arbitrarily small \citep{WanHic00b, Wan03a}. A key feature of
\texttt{QMCPy} is \textbf{stopping criteria} that automatically
determine \(n\) so \(|\mu - \widehat{\mu}| \le \varepsilon\) for a
user-specified tolerance \(\varepsilon>0\), either deterministically or
...
\item
  \textbf{Discrete Distributions} generate IID or LD sequences.
  Available LD sequences and randomization routines
  \citep{sorokin2025unified} include

  \begin{itemize}
  \tightlist
  \item
    \textbf{Lattices} with random shifts
    \citep{CraPat76, HicEtal03, Ric51, coveyou1967fourier, WanHic02a}.
  \item
    \textbf{Digital Sequences} (including Sobol' sequences) with digital
    shifts, linear matrix scrambling (LMS), or nested uniform scrambling
    (NUS, also called Owen scrambling)
    \citep{Sob67, dick2005multivariate, Mat98, Owe95, owen2003variance, dick2011higher, Nie87, Nie92, DicPil10a}.
  \item
    \textbf{Halton Sequences} with digital shifts, permutation
    scrambling, LMS, or NUS
    \citep{Hal60, WanHic00, Mat98, owen2024gain, MorCaf94, owen2024gain}.
  \end{itemize}

  We support higher-order digital sequences and higher order scrambling
  for integrands \(f\) with \(\alpha\) degrees of ``smoothness'',
  enabling QMC convergence like \(\mathcal{O}(n^{-\alpha+\delta})\)
  where \(\delta>0\) is arbitrarily small \citep{dick2011higher}.
  Internally, our LD generators call our C package \texttt{QMCToolsCL}
  \citep{QMCToolsCL}. \texttt{QMCPy} also integrates with the
  \texttt{LDData} repository \citep{LDData} which stores generating
  vectors for lattices and generating matrices for digital sequences,
  primarily drawing from the Magic Point Shop \citep{KuoNuy16a} and
  Kuo's websites on lattices
  \citep{cools2006constructing, nuyens2006fast, KuoGenerators} and
  Sobol' sequences
  \citep{JoeKuo03, joe2008constructing, SobolDirection}.
\item
  \textbf{True Measures} come with default transformations
  ...
  \Phi^{-1}(\mathbf{X}) + \mathbf{m}\) where \(\Phi^{-1}\) is the
  inverse distribution function of a standard normal applied
  elementwise. We support many measures, including those from
  \texttt{SciPy}'s \texttt{scipy.stats} \citep{2020SciPy-NMeth}.
\item
...
\item
  ...
  \(n\) until (Q)MC estimates satisfy user-defined error tolerances
  ...
  \tightlist
  \item
    multiple randomizations of LD sequences \citep{l2023confidence},
  \item
    quickly tracking the decay of Fourier coefficients
    \citep{HicJim16a, JimHic16a, HicEtal17a, DinHic20a}, or
  \item
    efficient Bayesian cubature by inducing structured Gram matrices
    \citep{Jag19a, RatHic19a, JagHic22a}.
  \end{itemize}

  \texttt{QMCPy} is also capable of simultaneously approximating
  functions of multiple integrands \citep{sorokin2022bounding}. Inspired
  by \texttt{MultilevelEstimators.jl} \citep{MultilevelEstimators},
  \texttt{QMCPy} is expanding support for multilevel (Q)MC SC
  \citep{giles2009multilevel, giles2015multilevel} that exploit cheaper,
  low-fidelity surrogates to accelerate expensive integrand estimates in
  high or infinite dimensions.
  ...
  The left panel shows sample complexity: MC algorithms require \(n =
   \mathcal{O}(1/\varepsilon^2)\) samples while QMC algorithms require
  only \(n =
   \mathcal{O}(1/\varepsilon)\) samples, with shaded regions showing
  10\%-90\% quantiles. The middle panel displays the computation time,
  highlighting that QMC methods are much faster than MC methods. The
  right panel presents error distributions via violin plots, showing
  that the average error performance is better for QMC methods.
\end{enumerate}

\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio,alt={MC and QMC SC comparison for pricing an Asian option. }]{../demos/talk_paper_demos/JOSS2025/JOSS2025.outputs/stopping_crit.png}
\caption{MC and QMC SC comparison for pricing an Asian option.
...
\citep{choi2023qmcpy} contains open-source code with continuous
integration, tests, and issue tracking
\citep{ChoEtal22a, sorokin2025unified}. \texttt{QMCPy} is distributed
under the Apache (v2.0) license. Community feedback and engagement are
welcome.
...
This article has been co-authored by employees of National Technology
and Engineering Solutions of Sandia, LLC under Contract No.~DE-NA0003525
with the U.S. Department of Energy (DOE). The employees co-own right,
title and interest in and to the article and are responsible for its
contents. The United States Government retains and the publisher, by
...
(\url{https://www.energy.gov/downloads/doe-public-access-plan}).
